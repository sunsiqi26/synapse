...
@main def main(): Unit = {
    ...
    println("[*] Starting Phase 1 BFS Orphan-Stitching Engine for SSTI...")
  ...
  val sinkFlask = cpg.call.methodFullName(".*render_template_string.*").l
  val sinkJinja2 = cpg.call.methodFullName(".*jinja2.*from_string.*|.*jinja2\\.Template.*").l
  val sinkMako = cpg.call.methodFullName(".*mako\\.template\\.Template.*").l
  val sinkDjango = cpg.call.methodFullName(".*django\\.template\\.Template.*").l
  val sinkAnsible = cpg.call.methodFullName("(?i).*(ansible_runner|runner)\\..*(run|apply|execute)$").l
  val sinkList = (sinkFlask ++ sinkJinja2 ++ sinkMako ++ sinkDjango ++ sinkAnsible).flatMap(_.method).distinct.l
